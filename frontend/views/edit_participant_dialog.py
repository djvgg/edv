...
class Edit_Participants(tk.Toplevel):
    ...
    def _init_participant_data(self):
        ...
        self.parent.logger.debug(f"EDIT_DIALOG: Opening dialog for bracket={self.bracket_key}, fighter_idx={self.fighter_idx}, is_quarantine={self.is_quarantine}")
        
        fighters = self.parent.brackets.get(self.bracket_key, {}).get('fighters', [])
        if not (0 <= self.fighter_idx < len(fighters)):
            self.parent.logger.error(f"EDIT_DIALOG: Invalid fighter_idx {self.fighter_idx} for bracket {self.bracket_key}")
            return False
            
        self.fighter = fighters[self.fighter_idx]
        self.parent.logger.debug(f"EDIT_DIALOG: Editing {self.fighter.get('Firstname', '')} {self.fighter.get('Lastname', '')} (ID: {self.fighter.get('ID', '?')})")
        ...
        if not self.gender and self.fighter.get('Gender'):
            ...
            if g:
                self.gender = g[0]
                self.parent.logger.debug(f"EDIT_DIALOG: Using fighter's Gender field: {self.gender}")
    ...
    def _save_changes(self):
        try:
            errors = []
            first_name_val = self.first_entry.get().strip()
            last_name_val = self.last_entry.get().strip()
            weight_raw = self.weight_entry.get().strip().replace(',', '.')
            birth_year_raw = self.birth_year_entry.get().strip()
            club_val = self.club_entry.get().strip()
            association_val = self.association_entry.get().strip()

            if not first_name_val: errors.append("Firstname must not be empty.")
            if first_name_val and first_name_val.endswith('-'): errors.append("Firstname must not end with a hyphen.")
            if last_name_val and last_name_val.endswith('-'): errors.append("Lastname must not end with a hyphen.")
            if not weight_raw or float(weight_raw) <= 0: errors.append("Weight must be greater than 0.")
            if birth_year_raw and (len(birth_year_raw) != 4 or not birth_year_raw.isdigit()):
                ...
                if not age_is_valid: errors.append(f"Invalid Age: {age_rejection_reason}")
                    
            if errors:
                messagebox.showwarning("Validation Error", "\n".join(errors), parent=self)
                return

            self.fighter['Firstname'] = first_name_val
            self.fighter['Lastname'] = last_name_val
            self.fighter['Weight'] = float(weight_raw)
            self.fighter['Club'] = club_val
            self.fighter['Association'] = association_val
            ...
            display_bracket_key = self.bracket_key
            ...
            if self.is_quarantine:
                if self.parent.quarantine_service:
                    ...
                    self.parent.quarantine_service.resort_brackets(self.parent.brackets, edited_fighter=self.fighter, group_preview_screen=self.parent)
                    
                    for bracket_key_check, bracket_data in self.parent.brackets.items():
                        if bracket_key_check.startswith('QUARANTINE_'): continue
                        for f in bracket_data.get('fighters', []):
                            if f.get('_tracking_id') == tracking_id:
                                target_bracket_key = bracket_key_check
                                break
                        if target_bracket_key: break
                    
                    for b_key, b_data in self.parent.brackets.items():
                        for f in b_data.get('fighters', []):
                            if '_tracking_id' in f: del f['_tracking_id']
                    
                    if self.bracket_key in self.parent.brackets and self.parent.brackets[self.bracket_key].get('fighters', []):
                        display_bracket_key = self.bracket_key
                ...
                if not self.is_adult_category:
                    ...
                    if new_ac != self.age_group:
                        ...
                        new_age_weight_classes = self._get_available_weight_classes(self.gender, effective_age)
                        if self.current_weight_class not in new_age_weight_classes:
                            if self.parent.config_repo and new_weight > 0:
                                detected_weight_class = self.parent.config_repo.get_weight_class(new_weight - self._group_tolerance, self.gender, effective_age)
                                if detected_weight_class and detected_weight_class != 'unknown':
                                    ...
                                    effective_weight_class = new_age_weight_classes[0]
                            elif new_age_weight_classes:
                                effective_weight_class = new_age_weight_classes[0]
                ...
                if self.is_adult_category:
                    ...
                    if self.parent.config_repo and new_weight > 0:
                        auto_detected_class = self.parent.config_repo.get_weight_class(new_weight - self._group_tolerance, self.gender, effective_age)
                    if auto_detected_class and dropdown_selection != auto_detected_class:
                        effective_weight_class = dropdown_selection
                        manual_override = True
                    elif dropdown_selection != self.current_weight_class:
                        effective_weight_class = dropdown_selection
                        manual_override = True
                ...
                if birth_year_val and len(birth_year_val) == 4 and birth_year_val.isdigit():
                    new_birth_year = int(birth_year_val)
                    calculated_age = datetime.datetime.now().year - new_birth_year
                    ...
                    if detected_age_group and detected_age_group != effective_age:
                        ...
                        if self.parent.config_repo and new_weight > 0:
                            detected_weight_class_for_age = self.parent.config_repo.get_weight_class(new_weight - self._group_tolerance, self.gender, effective_age)
                ...
                if effective_age != self.age_group or effective_weight_class != self.current_weight_class:
                    new_bracket_key = self.parent._move_participant_to_bracket(self.bracket_key, self.fighter_idx, self.gender, effective_age, effective_weight_class)
                    target_bracket_key = new_bracket_key
                    
                    old_bracket = self.parent.brackets.get(self.bracket_key, {})
                    if old_bracket and old_bracket.get('fighters', []): display_bracket_key = self.bracket_key
                    else: display_bracket_key = new_bracket_key

            db_svc = getattr(self.parent, 'db_service', None)
            if db_svc:
                db_svc.update_participant(self.fighter, display_bracket_key or 'QUARANTINE')

            self.fighter['Name'] = f"{first_name_val} {last_name_val}".strip()
            ...
            if hasattr(self.parent, 'flash_bracket'):
                ...
                if flash_key: self.parent.flash_bracket(flash_key)
            self.destroy()
        except ValueError:
